...
Similar to shiftlogic.py but instead it alternates back and forth around each
layer as opposed to spiralling around.  Note that this case does not add
bins in a connected manner.  Some is added to top and bottom.  This might not
perform as desired.

TODO: Add a version where the bins are connected.

The only thing that has thus changed here is the function 'order_disk_layer'.
...
def order_disk_layer(layer,R,origin):
	...
	# Get the mirror of quad0.
	quad4 = [i for i in [(-pt[0],pt[1]) for pt in quad0]]
	
	# Splice together quad0 and quad4 to create the alternating pattern.
	spliced = []
	for count,el in enumerate(quad0):
		if el%2:
